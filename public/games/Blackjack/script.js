...
class Blackjack {
    ...
    loadGame() {
        try {
            const saved = localStorage.getItem('cosmicBlackjackSave');
            if (saved) {
                const data = JSON.parse(saved);
                this.balance = data.balance ?? 1000;
                this.highScore = data.highScore ?? 1000;
                this.stats = data.stats ?? { gamesPlayed: 0, gamesWon: 0, blackjacks: 0 };
                if (this.balance <= 0) {
                    this.balance = 1000;
    ...
    saveGame() {
        ...
        try {
            localStorage.setItem('cosmicBlackjackSave', JSON.stringify({
