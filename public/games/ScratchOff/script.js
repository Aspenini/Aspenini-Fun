...
class ScratchOff {
    ...
    loadGame() {
        try {
            const saved = localStorage.getItem('cosmicScratchOffSave');
            if (saved) {
                const data = JSON.parse(saved);
                this.balance = data.balance ?? 1000;
                this.highScore = data.highScore ?? 1000;
                this.ticketNumber = data.ticketNumber ?? 1;
                this.stats = data.stats ?? { ticketsPlayed: 0, ticketsWon: 0, totalWinnings: 0, biggestWin: 0 };
                if (this.balance <= 0) {
                    this.balance = 1000;
    ...
    saveGame() {
        ...
        try {
            localStorage.setItem('cosmicScratchOffSave', JSON.stringify({
