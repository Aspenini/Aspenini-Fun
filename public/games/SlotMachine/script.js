...
class SlotMachine {
    ...
    loadGame() {
        try {
            const saved = localStorage.getItem('cosmicSlotsSave');
            if (saved) {
                const data = JSON.parse(saved);
                this.balance = data.balance ?? 1000;
                this.highScore = data.highScore ?? 1000;
                this.stats = data.stats ?? { totalSpins: 0, totalWins: 0, biggestWin: 0 };
                if (this.balance <= 0) {
                    this.balance = 1000;
    ...
    saveGame() {
        ...
        try {
            localStorage.setItem('cosmicSlotsSave', JSON.stringify({
